...
assert(/Co-Win/i.test(index) && /ORTAK GALİBİYET/.test(index), "home must explain Co-Win in EN/TR");
assert(/1\.5(?:×|x)/i.test(index), "home must explain the 1.5× reward");
assert(/100 victory XP/.test(index) && /eligible 7\+ day wager duels/.test(index) && /server rewards are active/.test(index), "Co-Win copy must retain backend eligibility and activation limits");
assert(/single winner receives the full Spark pot/.test(index), "single-winner copy must explain the full-pot payout");
...
assert(count(index, /<label[^>]+for=["'][^"']+["']/gi) >= 2, "waitlist email inputs must have persistent labels");
assert(/og:title[^>]+Win together, or outlast your rival/.test(index) && /twitter:title[^>]+Win together, or outlast your rival/.test(index), "social metadata must match current positioning");
